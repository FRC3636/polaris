...
if __name__ == "__main__":
    ...
    unsupported_object_platform = False
    ...
    if config.local_config.objdetect_enable:
        # check if /proc/device-tree/model contains "Rubik"
        with open("/proc/device-tree/model", "r") as f:
            model_str = f.read()
        if "Rubik" not in model_str:
            print("Object detection is only supported on Rubik Pi devices.")
            unsupported_object_platform = True
        else:
    ...
    while True:
        ...
        if calibration_command_source.get_calibrating(config):
            ...
            if config.local_config.objdetect_enable and not unsupported_object_platform:
